...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${USER_API}/api/users/login`, form);
    ...
    <div style={{minHeight:'80vh',display:'flex',alignItems:'center',justifyContent:'center',padding:'20px',background:'#fafafa'}}>
      <div style={{width:'100%',maxWidth:'420px'}}>
        <div style={{textAlign:'center',marginBottom:'32px'}}>
          ...
          <h1 style={{fontSize:'26px',fontWeight:'700',color:'#1a1a1a',marginBottom:'8px'}}>Welcome back</h1>
          <p style={{color:'#888',fontSize:'15px'}}>Sign in to continue to BlogHub</p>
        </div>
        ...
        <div style={{background:'#fff',borderRadius:'16px',padding:'32px',boxShadow:'0 4px 24px rgba(0,0,0,0.08)'}}>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{marginBottom:'16px'}}>
              ...
              <input
                ...
                required
                style={{width:'100%',padding:'11px 14px',border:'1px solid #e0e0e0',borderRadius:'8px',fontSize:'15px',outline:'none',transition:'border 0.2s',boxSizing:'border-box'}}
            ...
            <button type="submit" disabled={loading}
              style={{width:'100%',background:loading?'#aaa':'#1a8917',color:'#fff',border:'none',padding:'12px',borderRadius:'8px',fontSize:'16px',fontWeight:'600',cursor:loading?'not-allowed':'pointer',transition:'all 0.2s'}}>
          ...
          <p style={{textAlign:'center',marginTop:'20px',color:'#888',fontSize:'14px'}}>
            ...
            <Link to="/register" style={{color:'#1a8917',fontWeight:'600'}}>Create one</Link>
          </p>
        </div>
      </div>
    </div>
